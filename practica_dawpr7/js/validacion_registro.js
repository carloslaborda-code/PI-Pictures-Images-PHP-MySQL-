...
function validarEmail(email) {
    // Si el campo está vacío, retornamos un mensaje de error
    if (email.length === 0) {
        return "El correo electrónico no puede estar vacío.";
    }

    // Si el email excede los 254 caracteres, retornamos un mensaje de error
    if (email.length > 254) {
        return "La dirección de email no puede exceder los 254 caracteres.";
    }

    // Expresión regular para validar el email con las condiciones dadas
    ...
    - [a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]{1,64}: La parte local del email permite letras, números y ciertos caracteres especiales, con una longitud máxima de 64 caracteres.
    - @: Separador que divide la parte local del dominio.
    - [a-zA-Z0-9-]{1,63}: Un subdominio debe tener entre 1 y 63 caracteres y puede incluir letras, números y guiones.
    - (\.[a-zA-Z0-9-]{1,63}){1,255}: Permite uno o más subdominios separados por puntos, con las mismas restricciones que el primer subdominio. La longitud máxima total del dominio es 255 caracteres.
    ...
    */
    ...
    return ""; // Si todo está bien, retornamos una cadena vacía (sin errores)
}
